...
class MolliePosTerminalPayments(models.Model):
    ...
    def _mollie_get_pos_context_from_payment(self, mollie_payment):
        pos_config = mollie_payment.session_id.config_id if mollie_payment.session_id else False
        api_key = pos_config.mollie_terminal_api_key if pos_config else False
        if not api_key:
            raise ValidationError(_('Please configure a Mollie Terminal API key on the POS configuration.'))
    ...
    def mollie_cancel_payment_request(self, transaction_id=None, mollie_uid=None):
        ...
        if mollie_payment and mollie_payment.status == 'open':
            ...
            return mollie_payment.terminal_id.with_context(**ctx)._api_cancel_mollie_payment(mollie_payment.name)
        return {}
    ...
    def _mollie_process_webhook(self, webhook_data, order_type='pos', notify=True):
        ...
        if mollie_payment:
            ctx = self._mollie_get_pos_context_from_payment(mollie_payment)
            payment_status = mollie_payment.terminal_id.with_context(**ctx)._api_get_mollie_payment_status(webhook_data.get('id'))
            if payment_status and payment_status.get('status'):
                ...
                for refund_data in payment_status.get('_embedded', {}).get('refunds', []):
                    refund_payment = self.sudo().search([('name', '=', refund_data.get('id'))], limit=1)
