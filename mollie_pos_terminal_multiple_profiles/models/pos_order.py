...
class PosOrder(models.Model):
    ...
    def get_mollie_payment_details(self, pos_session_id):
        mollie_line = self.payment_ids.filtered(
            lambda line: line.transaction_id and line.payment_method_id.use_payment_terminal == 'mollie'
        )
        if len(mollie_line) == 1:
            ...
            terminal = mollie_line.payment_method_id.mollie_pos_terminal_id.with_context(
                mollie_api_key=api_key,
                pos_config_id=pos_config.id,
            )
            mollie_payment_data = terminal._api_get_mollie_payment_status(mollie_line.transaction_id)
